    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {attempts.map((attempt) => {
          // Get a safe date for display
          const displayDate = attempt.completedAt 
            ? new Date(attempt.completedAt).toLocaleDateString()
            : 'Not completed';
          
          // Get started date as fallback
          const startedDate = attempt.startedAt 
            ? new Date(attempt.startedAt).toLocaleDateString()
            : 'Date not available';

          return (
            <Card key={attempt.id.toString()}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold">{attempt.studentName}</h4>
                    <p className="text-sm text-gray-600">
                      {attempt.status === 'completed' ? 'Completed' : attempt.status || 'In Progress'}: {displayDate}
                    </p>
                  </div>
